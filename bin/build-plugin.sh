...
mkdir -p "${BUILD_DIR}/wmcp-agentops" "${DIST_DIR}"
rsync -a \
  --exclude='.DS_Store' \
  --exclude='*.map' \
  --exclude='vendor' \
  "${PLUGIN_SOURCE}/" "${BUILD_DIR}/wmcp-agentops/"

cp "${REPO_DIR}/LICENSE" "${BUILD_DIR}/wmcp-agentops/LICENSE"
if [[ -f "${REPO_DIR}/THIRD_PARTY_NOTICES.md" ]]; then
  cp "${REPO_DIR}/THIRD_PARTY_NOTICES.md" "${BUILD_DIR}/wmcp-agentops/THIRD_PARTY_NOTICES.md"
fi

if find "${BUILD_DIR}/wmcp-agentops" \
...
if grep -RIlE -- \
  ...
  "${BUILD_DIR}/wmcp-agentops" >/dev/null; then
  echo "A secret-like credential was found in the plugin package." >&2
  exit 1
fi

find "${BUILD_DIR}/wmcp-agentops" -exec touch -t 198001010000 {} +

ARCHIVE="${DIST_DIR}/wmcp-agentops-${VERSION}.zip"
rm -f "${ARCHIVE}" "${ARCHIVE}.sha256"
(
  cd "${BUILD_DIR}"
  find wmcp-agentops -print | LC_ALL=C sort | zip -X -q "${ARCHIVE}" -@
)
