...
use WPWebMCP\AgentSNR\WooCommerce\SessionCorrelator;
...
final class DemoReset
{
    ...
    public function handle(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (! in_array($surface, array('storefront', 'agentsnr'), true)) {
            return $this->responses->make(array('ok' => false, 'error' => array('code' => 'invalid_surface', 'message' => 'Choose storefront or agentsnr.')), 400);
        }
        ...
        if (false === wp_next_scheduled('wmcp_agentsnr_cleanup_session', array($current['hash_hex']))) {
