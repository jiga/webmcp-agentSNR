...
final class DemoPages
{
    ...
    public function register_assets(): void
    {
        ...
        wp_register_script('wmcp-surface-runtime', $base . 'assets/js/agentsnr-runtime.js', array('wmcp-webmcp-runtime'), $version, true);
     ...
     * @param array<string, mixed> $attributes Shortcode attributes.
     */
    public function render_agentsnr(array $attributes = array()): string
    {
        unset($attributes);
        $this->enqueue_surface('agentsnr');
        $view               = $this->base_view();
        ...
        return $this->render_template('agentsnr-demo.php', $view);
    }
    ...
    private function enqueue_surface(string $surface): void
    {
        ...
        } elseif ('agentsnr' === $surface) {
            wp_enqueue_script('wmcp-dashboard');
        }

        $runtime_surface = 'agentsnr' === $surface ? 'agentsnr' : 'storefront';
        $config          = $this->runtime_config($runtime_surface);
        $config['autoStart'] = in_array($surface, array('storefront', 'agentsnr'), true);
    ...
    private function base_view(): array
    {
        return array(
            ...
            'agentsnr_url'   => home_url('/agentsnr-demo/'),
            'health_url'     => home_url('/webmcp-health/'),
            'playground_url' => $this->optional_external_url('wmcp_agentsnr_playground_url'),
            'repository_url' => $this->optional_external_url('wmcp_agentsnr_repository_url'),
            'release_url'    => $this->optional_external_url('wmcp_agentsnr_release_url'),
    ...
    {
        $path = (defined('WMCP_AGENTSNR_PATH') ? WMCP_AGENTSNR_PATH : dirname(__DIR__, 2) . '/') . 'templates/' . $template;
        if (! is_readable($path)) {
            return '';
        }
