...
/**
 * Authenticated Agent SNR admin dashboard renderer.
 *
...
final class DashboardController
{
    ...
    public function render(): void
    {
        if (! current_user_can('view_wmcp_analytics')) {
            wp_die(esc_html__('You do not have permission to view agent-monitoring analytics.', 'wmcp-agentsnr'), '', array('response' => 403));
        }
        ...
        $template         = (defined('WMCP_AGENTSNR_PATH') ? WMCP_AGENTSNR_PATH : dirname(__DIR__, 2) . '/') . 'templates/agentsnr-demo.php';

        if (! is_readable($template)) {
            echo '<div class="notice notice-error"><p>' . esc_html__('The Agent SNR monitor template is unavailable.', 'wmcp-agentsnr') . '</p></div>';
            return;
        }
