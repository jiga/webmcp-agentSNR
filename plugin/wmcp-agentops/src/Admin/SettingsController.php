...
final class SettingsController
{
    ...
    {
        add_action('admin_post_wmcp_agentops_save_settings', array($this, 'save'));
    }

    public function save(): void
    {
        if (! current_user_can('manage_wmcp_policies')) {
            wp_die(esc_html__('You do not have permission to manage WebMCP policies.', 'wmcp-agentops'), '', array('response' => 403));
        }

        check_admin_referer('wmcp_agentops_save_settings');
        ...
        update_option('wmcp_agentops_enabled', $plugin_after, false);
    ...
    private function audit(string $event_name, string $target, bool $before, bool $after): void
    {
        ...
        try {
            ...
            $workflow   = $this->workflows->current($scope_hash, 'agentops');
            $this->events->record(
                (string) $workflow['id'],
                $event_name,
                array(
