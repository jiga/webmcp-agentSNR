...
final class CartService
{
    ...
    private function revision(object $cart): string
    {
        ...
        $revision = method_exists($session, 'get') ? (int) $session->get('wmcp_agentops_cart_revision', 0) : 0;
    ...
    private function bump_revision(): void
    {
        ...
        if (method_exists($session, 'get') && method_exists($session, 'set')) {
            $session->set('wmcp_agentops_cart_revision', ((int) $session->get('wmcp_agentops_cart_revision', 0)) + 1);
        }
    }
}
