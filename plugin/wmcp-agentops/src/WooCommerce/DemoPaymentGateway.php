 ...
 *
 * Loaded only when WooCommerce is active and WMCP_AGENTOPS_DEMO_MODE is true.
 *
...
final class DemoPaymentGateway extends \WC_Payment_Gateway
{
    ...
    {
        $this->id                 = 'wmcp_agentops_demo';
        $this->method_title       = __('Demo payment — no charge', 'wmcp-agentops');
        $this->method_description = __('Completes a demo order without contacting a payment processor or collecting card details.', 'wmcp-agentops');
        $this->title              = __('Demo payment — no charge', 'wmcp-agentops');
        $this->description        = __('For this public demo only. No payment is processed and no card details are requested or stored.', 'wmcp-agentops');
    ...
    public function process_payment($order_id)
    {
        ...
        if (
            ...
            if (function_exists('wc_add_notice')) {
                wc_add_notice(__('The no-charge demo checkout requires a current agent-prepared cart and an available demo quota.', 'wmcp-agentops'), 'error');
            }
            return array('result' => 'failure', 'redirect' => '');
        }
        ...
        if (! $this->consume_quota($session['hash_hex'], $order)) {
            if (function_exists('wc_add_notice')) {
                wc_add_notice(__('The no-charge demo order limit has been reached. Start a fresh run later.', 'wmcp-agentops'), 'error');
            }
        ...
        $order->add_order_note(__('Demo payment completed. No money was charged.', 'wmcp-agentops'));
