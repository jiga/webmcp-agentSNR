...
( function () {
	...
	const DEFAULTS = Object.freeze( {
		broadcastChannelName: "wmcp-agentops:manifest-invalidation",
		invalidationStorageKey: "wmcp-agentops:manifest-invalidation",
	...
	function discoverConfig( browserWindow, browserDocument ) {
		...
		const surface = declaredSurface === "agentops" ? "agentops" : "storefront";
		...
		try {
			const namespace = new URL( "wmcp-agentops/v1/", apiRoot ).toString();

			return {
				autoStart: [ "storefront", "agentops" ].includes( declaredSurface ),
	...
	if ( typeof window === "object" ) {
		window.WMCPAgentOps = Object.assign( {}, window.WMCPAgentOps || {}, api );
		bootstrap( window );
	}
}() );
